...
def vip_tel_no():
    ...
    while True:
        if len(vip_tel) == 11:
            ...
            vip_tel = input('你输入的手机号不正确，请重新输入：')
    return vip_tel
...
def vip_check(vip_dict):
    ...
    if vip_no in vip_dict.keys():
        ...
        vip_tel = vip_tel_no()
        vip_info_list.append(vip_tel)
        vip_info_list.append(input('请输入生日（yyyymmdd）：'))
        vip_info_list.append(input('请输入性别(m/f)：'))
        vip_info_list.append(input('请输入星座：'))
        vip_info_list.append(input('请输入所在地：'))
        vip_dict['vip_no'] = vip_no
        vip_dict['info'] = vip_info_list
        vip_dict['date'] = get_date('%Y-%m-%d %H:%M:%S')
        header = ['vip_no', 'info', 'date']
        write_csv('vip_info.csv', header, [vip_dict])
        vip = False
    return vip_no, vip
...
def shopping_log(shopping_car, sold_log_list, vip_no):
    for milktea_type, milktea_counts in shopping_car.items():
        ...
        sold_log['date'] = get_date('%Y-%m-%d %H:%M:%S')
...
def write_csv(log_file_name, header, data, path=''):
    with open(path + log_file_name, 'a', encoding='utf-8', newline='') as f:
...
def main():
    ...
    i = 1
    print('今天是{}，营业的第{}天。'.format(today, i))
    while i <= 30:
        vistor = 0
        sold_log_list = []
        while vistor < 5:
        ...
        log_file_name = 'sold_log_' + get_date() + '.csv'
        header = ['vip_no', 'milktea_type', 'milktea_counts', 'date']
        write_csv(log_file_name, header, sold_log_list, path='log/')
