...
def user_buy_log_manager_sys(user_id, *buy_car):
    buy_log_dict = {}
    buy_log_dict['user_id'] = user_id
    buy_log_dict['money'] = buy_car_account(buy_car, user_id)
    buy_log_dict['items'] = buy_car
        ...
        for log in g_user_buy_log_list:
            ...
            for value_set in other_user_item_dict.values():
                ...
                if length > 0 and length < len(value_set):
                    ...
                    recommend_list.append({'common_num': length, 'diff_items': diff_set})
            if len(recommend_list) > 0:
                recommend_list.sort(key=lambda items: items['common_num'], reverse=True)
